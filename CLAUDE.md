...
Production runs on a server reachable via the `adt` SSH alias at `/home/django/web-electro`, on the `main` branch (gunicorn + nginx + pm2). The `prod` branch on GitHub is stale/abandoned (tens of commits behind `main`) — don't treat it as the source of truth for what's deployed; `main` is what's actually running. When comparing local state against production, prefer diffing against `origin/main`.
